...
--
-- Generic-domain negative testbench for lm_math_fi_delay.
--
-- THIS DESIGN IS EXPECTED TO FAIL, once per case.
--
-- The generics below mirror lm_math_fi_delay's own generics: same names, same subtypes,
-- and defaults that are all legal. A case is selected by overriding exactly one
-- of them from the command line, for example:
--
--   ghdl -r tb_neg_delay -g<generic>=<illegal value>
--
...
entity tb_neg_delay is
  generic(
    g_delay  : natural  := 1;
    g_data_w : positive := 4
    );
end entity tb_neg_delay;

architecture a_tb of tb_neg_delay is
...
begin
  ...
  dut : entity lm_math_fi_lib.lm_math_fi_delay
  ...
  begin
    wait for 50 ns;
    report "GENERIC DOMAIN TB COMPLETED: tb_neg_delay" severity note;
    wait;
