...
--
-- Generic-domain negative testbench for lm_math_fi_mult.
--
-- THIS DESIGN IS EXPECTED TO FAIL, once per case.
--
-- The generics below mirror lm_math_fi_mult's own generics: same names, same subtypes,
-- and defaults that are all legal. A case is selected by overriding exactly one
-- of them from the command line, for example:
--
--   ghdl -r tb_neg_mult -g<generic>=<illegal value>
--
...
entity tb_neg_mult is
  generic(
    ...
    g_din_a_type   : natural  := C_LM_UNSIGNED;
    g_din_b_type   : natural  := C_LM_UNSIGNED;
    g_dout_type    : natural  := C_LM_UNSIGNED;
    g_overflow     : natural  := C_LM_WRAP;
    g_pipe_stages  : natural  := 0
    );
end entity tb_neg_mult;

architecture a_tb of tb_neg_mult is
...
begin
  ...
  dut : entity lm_math_fi_lib.lm_math_fi_mult
    generic map(
      ...
      g_din_a_type => g_din_a_type, g_din_b_type => g_din_b_type,
      g_dout_type => g_dout_type,
      g_overflow => g_overflow, g_pipe_stages => g_pipe_stages)
  ...
  begin
    report "GENERIC DOMAIN TB STARTED: tb_neg_mult" severity note;
    wait;
  ...
  begin
    wait for 50 ns;
    report "GENERIC DOMAIN TB COMPLETED: tb_neg_mult" severity note;
    wait;
