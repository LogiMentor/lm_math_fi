...
-- lm_math_fi_mult with a negative g_pipe_stages. This unit must be rejected by
-- GHDL ANALYSIS, not by a run-time assertion: g_pipe_stages is a natural, so
-- the value is out of the generic's subtype. Before g_pipe_stages was
-- narrowed from integer to natural this analysed and elaborated cleanly and
-- then died with an index-out-of-bounds error inside the library.
--
...
entity tb_neg_mult_pipe_stages_negative is
end entity tb_neg_mult_pipe_stages_negative;

architecture a_tb of tb_neg_mult_pipe_stages_negative is
...
begin
  ...
  dut : entity lm_math_fi_lib.lm_math_fi_mult
    generic map(
      ...
      g_overflow => C_LM_WRAP, g_pipe_stages => -1)
    port map(clk_i => clk_tb, ce_i => '1', din1_i => s_a, din2_i => s_b, dout_o => s_dout);

  proc_guard : process
  begin
    wait for 50 ns;
    report "NEGATIVE TEST DID NOT FAIL: tb_neg_mult_pipe_stages_negative elaborated and ran without the expected diagnostic"
      severity failure;
    wait;
