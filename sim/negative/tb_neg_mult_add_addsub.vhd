...
-- lm_math_fi_mult_add asked for C_LM_ADDSUB, which it does not implement.
-- Before this check existed neither generate branch was selected and the
-- result port stayed undriven at all-U.
--
...
entity tb_neg_mult_add_addsub is
end entity tb_neg_mult_add_addsub;

architecture a_tb of tb_neg_mult_add_addsub is
...
begin
  ...
  dut : entity lm_math_fi_lib.lm_math_fi_mult_add
    generic map(
      ...
      g_add_sub => C_LM_ADDSUB, g_round_mode => C_LM_TRUNC_BITS,
  ...
  begin
    wait for 50 ns;
    report "NEGATIVE TEST DID NOT FAIL: tb_neg_mult_add_addsub elaborated and ran without the expected diagnostic"
      severity failure;
    wait;
