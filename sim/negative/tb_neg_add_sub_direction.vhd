...
-- lm_math_fi_add_sub with a g_direction that is not C_LM_ADD, C_LM_SUB or
-- C_LM_ADDSUB.
--
...
entity tb_neg_add_sub_direction is
end entity tb_neg_add_sub_direction;

architecture a_tb of tb_neg_add_sub_direction is
...
begin
  ...
  dut : entity lm_math_fi_lib.lm_math_fi_add_sub
    generic map(
      g_direction => 7, g_representation => C_LM_UNSIGNED,
  ...
  begin
    wait for 50 ns;
    report "NEGATIVE TEST DID NOT FAIL: tb_neg_add_sub_direction elaborated and ran without the expected diagnostic"
      severity failure;
    wait;
