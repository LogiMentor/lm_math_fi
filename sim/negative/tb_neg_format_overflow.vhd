...
-- lm_math_fi_format with a g_overflow that is neither C_LM_SATURATE nor C_LM_WRAP.
--
...
entity tb_neg_format_overflow is
end entity tb_neg_format_overflow;

architecture a_tb of tb_neg_format_overflow is
...
begin
  ...
  dut : entity lm_math_fi_lib.lm_math_fi_format
    generic map(
      ...
      g_overflow => 0, g_representation => C_LM_UNSIGNED)
    port map(clk_i => clk_tb, din_i => s_din, dout_o => s_dout);

  proc_guard : process
  begin
    wait for 50 ns;
    report "NEGATIVE TEST DID NOT FAIL: tb_neg_format_overflow elaborated and ran without the expected diagnostic"
      severity failure;
    wait;
